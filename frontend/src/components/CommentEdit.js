    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          ...
          response.status === 403 ? 'You are not authorized to edit this comment' :
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          response.status === 404 ? 'Comment not found' :
          response.status === 403 ? 'You are not authorized to delete this comment' :
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginTop: '0.5rem' }}>
      ...
      {Array.isArray(comments) && comments.length > 0 ? (
        comments.map((comment) => {
          ...
          const createdAt = comment.createdAt && !isNaN(new Date(comment.createdAt))
          ...
          return (
            ...
            >
              <p style={{ fontSize: '0.7rem', color: '#aaa', margin: '0 0 0.2rem 0' }}>
              ...
              {editing[comment.COMMENT_ID] ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <textarea
                    ...
                    onChange={(e) => setEditContent((prev) => ({ ...prev, [comment.COMMENT_ID]: e.target.value }))}
                    style={{
                      width: '100%',
                      minHeight: '60px',
                      padding: '0.5rem',
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button
                      ...
                      style={{
                        ...
                        color: 'white',
                        cursor: deleting[comment.COMMENT_ID] ? 'not-allowed' : 'pointer',
                        fontSize: '0.7rem',
                      }}
                    >
                      Save
                    ...
                    <button
                      onClick={() => setEditing((prev) => ({ ...prev, [comment.COMMENT_ID]: false }))}
                      style={{
                        backgroundColor: '#888',
                        border: 'none',
                        borderRadius: '5px',
                ...
                <>
                  ...
                  {canEdit && (
                    <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.3rem' }}>
                      <button
                        onClick={() => startEditing(comment.COMMENT_ID, comment.COMMENT_CONTENT)}
                        disabled={deleting[comment.COMMENT_ID]}
                        style={{
                          backgroundColor: '#4CAF50',
                          border: 'none',
